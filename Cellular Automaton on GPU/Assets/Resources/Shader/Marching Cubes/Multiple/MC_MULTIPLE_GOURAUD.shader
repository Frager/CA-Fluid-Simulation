Shader "MarchingCubes/Multiple/Gouraud"
{
	Properties
	{
	...
	SubShader
	{
		...
		Pass
		{
			...
			fixed4 BlinnPhong(float4 position, float3 normal)
			{
				...
				fixed3 viewDirection = normalize(_WorldSpaceCameraPos - position.xyz);
			...
			void geom(point GS_INPUT p[1], inout TriangleStream<PS_INPUT> triStream)
			{
				...
				for (int i = 2; i >= 0; --i)
				{
					...
					pIn.color = p[0].color * BlinnPhong(mul(unity_ObjectToWorld, p[0].positions[i] * scale + offset), p[0].normals[i]);
